 ...
 */
class Noor_Starter_Templates_Give_Import {
		...
		if ( is_wp_error( $results ) ) {
			...
			if ( apply_filters( 'noor_starter_templates_save_log_files', false ) ) {
				...
				Helpers::append_to_file(
					$error_message,
					$log_file_path,
					esc_html__( 'Importing Give forms', 'noor-starter-templates' )
				);
			}
		} else {
			$message = ( ! empty( $results['message'] ) ? $results['message'] : esc_html__( 'No results for Give Form import!', 'noor-starter-templates' ) );
			if ( apply_filters( 'noor_starter_templates_save_log_files', false ) ) {
		...
		if ( is_wp_error( $results ) ) {
			...
			if ( apply_filters( 'noor_starter_templates_save_log_files', false ) ) {
				...
				Helpers::append_to_file(
					$error_message,
					$log_file_path,
					esc_html__( 'Importing Give Donations', 'noor-starter-templates' )
				);
			}
		} else {
			$message = ( ! empty( $results['message'] ) ? $results['message'] : esc_html__( 'No results for Give import!', 'noor-starter-templates' ) );
			if ( apply_filters( 'noor_starter_templates_save_log_files', false ) ) {
				// Add this message to log file.
				$log_added = Helpers::append_to_file(
					$message,
	...
	private static function process_import_form_file( $file ) {
		...
		if ( ! file_exists( $file ) ) {
			return new \WP_Error(
				...
				__( 'Error: Form import file could not be found.', 'noor-starter-templates' )
			);
		}
	...
	public static function import_data( $raw_data ) {
		...
		if ( empty( $raw_data ) ) {
			return new \WP_Error(
				...
				__( 'Error: Give import data could not be read. Please try a different file.', 'noor-starter-templates' )
			);
		}
		...
		$results = array(
			'message' => __( 'Give data has been successfully imported.', 'noor-starter-templates' ),
		);
		// Return results.
		return apply_filters( 'noor-starter-templates/give_import_results', $results );
	}
}
