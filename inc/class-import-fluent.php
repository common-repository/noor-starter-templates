 ...
 */
class Noor_Starter_Templates_Fluent_Import {
		...
		if ( is_wp_error( $results ) ) {
			...
			if ( apply_filters( 'noor_starter_templates_save_log_files', false ) ) {
				...
				Helpers::append_to_file(
					$error_message,
					$log_file_path,
					esc_html__( 'Importing forms', 'noor-starter-templates' )
				);
			}
		} else {
			ob_start();
	...
	public static function import_data( $forms ) {
		...
		if ( empty( $forms ) ) {
			return new \WP_Error(
				...
				__( 'Error: Form import data could not be read. Please try a different file.', 'noor-starter-templates' )
			);
		}
		...
		if ( $forms && is_array( $forms ) ) {
			...
			// Return results.
			return apply_filters( 'noor-starter-templates/form_import_results', $results );
		}
	}
	/**
	...
	private static function format_results_for_log( $results ) {
		if ( empty( $results ) ) {
			esc_html_e( 'No results for form import!', 'noor-starter-templates' );
		}
