...
class Template_Database_Importer {
	 ...
	 * @var string
	 */
	protected $remote_url = 'https://docs.pixeldima.com/wp-json/jet-cct/noor_starter';
	...
	public function __construct() {
		if ( is_admin() ) {
			// Ajax Calls.
			add_action( 'wp_ajax_noor_import_get_template_data', array( $this, 'template_data_ajax_callback' ) );
			add_action( 'wp_ajax_noor_import_reload_template_data', array( $this, 'template_data_reload_ajax_callback' ) );
		}
	...
	public function get_template_data( $skip_local = false ) {
		if ( 'custom' === $this->template_type ) {
			return wp_json_encode( apply_filters( 'noor_starter_templates_custom_array', array() ) );
		}
	...
	public function get_remote_url_contents() {
		$site_url = '';
		$args     = apply_filters(
			'noor_starter_get_templates_args',
			array(
				'type'      => ( $this->template_type ? $this->template_type : 'blocks' ),
		...
		foreach ( $array as &$arr ) {
			 $newpage = $newmenu = array();

			foreach ( $arr['pages'] as &$page ) {
				$newpage[ $page['id'] ] = $page;
			}
			...
			$arr['pages']            = $newpage;
			$arr['menus']            = $newmenu;
			$newarray[ $arr['_ID'] ] = $arr;
		}
		$array = json_encode( $newarray );
	...
	public function get_local_template_data_filename() {
		$dima_api = 'free';
		$slug     = 'noor';
		$ntp_data = get_option( $slug . '_purchase_code' );
		if ( $ntp_data && ! empty( $ntp_data ) ) {
			$dima_api = $ntp_data;
		}

		return md5( $this->get_base_url() . $this->get_base_path() . $this->template_type . NOOR_STARTER_TEMPLATES_VERSION . $dima_api );
	}
	/**
	...
	public function template_data_ajax_callback() {
		...
		if ( ! $get_data ) {
			...
			wp_send_json( esc_html__( 'No template data', 'noor-starter-templates' ) );
		} else {
	...
	public function get_subfolder_name() {
		if ( ! $this->subfolder_name ) {
			$this->subfolder_name = apply_filters( 'noor_starter_templates_local_data_subfolder_name', 'noor_starter_templates' );
		}
	...
	public function get_base_path() {
		if ( ! $this->base_path ) {
			...
			$this->base_path = apply_filters( 'noor_starter_templates_local_data_base_path', trailingslashit( $upload_dir['basedir'] ) );
		}
	...
	public function get_base_url() {
		if ( ! $this->base_url ) {
			$this->base_url = apply_filters( 'noor_starter_templates_local_data_base_url', content_url() );
		}
