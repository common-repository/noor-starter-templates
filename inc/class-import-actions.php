...
class ImportActions {
	...
	public function register_hooks() {
		...
		add_action( 'noor-starter-templates/before_content_import_execution', array( $this, 'before_content_import_action' ), 10, 5 );

		// After content import.
		add_action( 'noor-starter-templates/after_content_import_execution', array( $this, 'before_widget_import_action' ), 10, 5 );
		add_action( 'noor-starter-templates/after_content_import_execution', array( $this, 'widgets_import' ), 20, 5 );

		add_action( 'noor-starter-templates/after_content_import_execution', array( $this, 'forms_import' ), 20, 5 );
		add_action( 'noor-starter-templates/after_content_import_execution', array( $this, 'donations_import' ), 20, 5 );
		add_action( 'noor-starter-templates/after_content_import_execution', array( $this, 'give_forms_import' ), 20, 5 );

		add_action( 'noor-starter-templates/after_content_import_execution', array( $this, 'revslider_import' ), 20, 5 );

		// Customizer import.
		add_action( 'noor-starter-templates/customizer_import_execution', array( $this, 'customizer_import' ), 10, 1 );

		// Customizer Color Only import.
		add_action( 'noor-starter-templates/customizer_import_color_only_execution', array( $this, 'customizer_import_color_only' ), 10, 1 );

		// Customizer font Only import.
		add_action( 'noor-starter-templates/customizer_import_font_only_execution', array( $this, 'customizer_import_font_only' ), 10, 1 );

		// After full import action.
		add_action( 'noor-starter-templates/after_all_import_execution', array( $this, 'after_import_action' ), 10, 5 );

		// Special widget import cases.
		add_action( 'noor-starter-templates/widget_settings_array', array( $this, 'fix_custom_menu_widget_ids' ) );
	}
	 ...
	 * @param array $import_files          The filtered import files defined in `noor-starter-templates/import_files` filter.
	...
	public function forms_import( $selected_import_files, $import_files, $selected_index, $selected_palette, $selected_font ) {
		if ( ! empty( $selected_import_files['forms'] ) && class_exists( 'Noor_Starter_Templates\Noor_Starter_Templates_Fluent_Import' ) ) {
			Noor_Starter_Templates_Fluent_Import::import( $selected_import_files['forms'] );
		}
	}
	 ...
	 * @param int   $selected_index        Selected index of import.
	 */
	public function revslider_import( $selected_import_files, $import_files, $selected_index, $selected_palette, $selected_font ) {
		if ( ! empty( $selected_import_files['revslider'] ) && class_exists( 'Noor_Starter_Templates\Noor_Starter_Templates_RevSlider_Import' ) ) {
			Noor_Starter_Templates_RevSlider_Import::import( $selected_import_files['revslider'] );
		}
	}
	...
	public function donations_import( $selected_import_files, $import_files, $selected_index, $selected_palette, $selected_font ) {
		if ( ! empty( $selected_import_files['give-donations'] ) && class_exists( 'Noor_Starter_Templates\Noor_Starter_Templates_Give_Import' ) ) {
			Noor_Starter_Templates_Give_Import::import( $selected_import_files['give-donations'] );
		}
	}
	/**
	...
	public function give_forms_import( $selected_import_files, $import_files, $selected_index, $selected_palette, $selected_font ) {
		if ( ! empty( $selected_import_files['give-forms'] ) && class_exists( 'Noor_Starter_Templates\Noor_Starter_Templates_Give_Import' ) ) {
			Noor_Starter_Templates_Give_Import::import_forms( $selected_import_files['give-forms'] );
		}
	}
	/**
	...
	/**
	 * Execute the action: 'noor-starter-templates/before_content_import'.
	 *
	 * @param array $selected_import_files Actual selected import files (content, widgets, customizer, redux).
	 * @param array $import_files          The filtered import files defined in `noor-starter-templates/import_files` filter.
	 * @param int   $selected_index        Selected index of import.
	...
	public function before_content_import_action( $selected_import_files, $import_files, $selected_index, $selected_palette, $selected_font ) {
		$this->do_import_action( 'noor-starter-templates/before_content_import', $import_files[ $selected_index ], $selected_palette, $selected_font );
	}


	/**
	 * Execute the action: 'noor-starter-templates/before_widgets_import'.
	 *
	 * @param array $selected_import_files Actual selected import files (content, widgets, customizer, redux).
	 * @param array $import_files          The filtered import files defined in `noor-starter-templates/import_files` filter.
	 * @param int   $selected_index        Selected index of import.
	...
	public function before_widget_import_action( $selected_import_files, $import_files, $selected_index, $selected_palette, $selected_font ) {
		$this->do_import_action( 'noor-starter-templates/before_widgets_import', $import_files[ $selected_index ], $selected_palette, $selected_font );
	}


	/**
	 * Execute the action: 'noor-starter-templates/after_import'.
	 *
	 * @param array $selected_import_files Actual selected import files (content, widgets, customizer, redux).
	 * @param array $import_files          The filtered import files defined in `noor-starter-templates/import_files` filter.
	 * @param int   $selected_index        Selected index of import.
	...
	public function after_import_action( $selected_import_files, $import_files, $selected_index, $selected_palette, $selected_font ) {
		$this->do_import_action( 'noor-starter-templates/after_import', $import_files[ $selected_index ], $selected_palette, $selected_font );
	}
	 ...
	 * @param array  $selected_import The data of selected import from `noor-starter-templates/import_files` filter.
	 */
	private function do_import_action( $action, $selected_import, $selected_palette, $selected_font ) {
		if ( false !== has_action( $action ) ) {
			$noor_starter_templates = Starter_Templates::get_instance();
			$log_file_path          = $noor_starter_templates->get_log_file_path();
