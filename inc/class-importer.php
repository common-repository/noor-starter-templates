...
class Importer {
	...
	/**
	 * The instance of the Noor_Starter_Templates\Logger class.
	 *
	 * @var object
	 */
	public $logger;
	...
	/**
	 * The instance of the Noor_Starter_Templates class.
	 *
	 * @var object
	 */
	private $noor_starter_templates;
	/**
	...
	public function __construct( $importer_options = array(), $logger = null ) {
		...
		// Get the noor_starter_templates (main plugin class) instance.
		$this->noor_starter_templates = Starter_Templates::get_instance();
	}
	...
	private function include_required_files() {
		...
		if ( ! class_exists( '\AwesomeMotive\WPContentImporter2\WXRImporter' ) ) {
			require_once NOOR_STARTER_TEMPLATES_PATH . 'vendor/wxr-importer/WXRImporter.php';
		}
		if ( ! class_exists( '\AwesomeMotive\WPContentImporter2\WXRImportInfo' ) ) {
			require_once NOOR_STARTER_TEMPLATES_PATH . 'vendor/wxr-importer/WXRImportInfo.php';
		}
		if ( ! class_exists( '\AwesomeMotive\WPContentImporter2\Importer' ) ) {
			require_once NOOR_STARTER_TEMPLATES_PATH . 'vendor/wxr-importer/Importer.php';
		}
		require_once NOOR_STARTER_TEMPLATES_PATH . 'inc/class-wxr-importer.php';
	}
	...
	public function import_content( $import_file_path, $single_page = false, $page_meta = '', $elementor = false ) {
		...
		if ( strpos( ini_get( 'disable_functions' ), 'set_time_limit' ) === false ) {
			set_time_limit( apply_filters( 'noor-starter-templates/set_time_limit_for_demo_data_import', 300 ) );
		}
		...
		if ( $single_page ) {
			...
			// add_action( 'wxr_importer.processed.post', array( $this, 'process_noor_galleries' ), 10, 5 );
		}

		// Disables generation of multiple image sizes (thumbnails) in the content import step.
		if ( ! apply_filters( 'noor-starter-templates/regenerate_thumbnails_in_content_import', true ) ) {
	...
	public function process_elementor( $post_id, $data, $meta, $comments, $terms ) {
		...
		if ( in_array( '_elementor_data', $meta_data, true ) ) {
			if ( class_exists( '\Elementor\TemplateLibrary\Noor_Starter_Templates_Elementor_Import' ) ) {
				$el_import = new \Elementor\TemplateLibrary\Noor_Starter_Templates_Elementor_Import();
	...
	public function process_internal_links( $data, $meta, $comments, $terms ) {
		if ( ! empty( $data['post_content'] ) && has_blocks( $data['post_content'] ) ) {
			...
			if ( ! empty( $all_links ) ) {
				...
				$demo_data = get_option( '_noor_starter_templates_last_import_data', array() );
	 ...
	 * @param array $terms Raw term data, already processed.
	 */
	public function process_noor_galleries( $post_id, $data, $meta, $comments, $terms ) {
		$meta_data = wp_list_pluck( $meta, 'key' );
		if ( in_array( '_elementor_data', $meta_data, true ) ) {
			return;
		}
	...
	public function check_for_content_images( $data, $meta, $comments, $terms ) {
		...
		if ( $time > apply_filters( 'noor-starter-templates/time_for_one_ajax_call', 25 ) ) {
			$response = array(
				'status'  => 'newAJAX',
			...
			if ( ! empty( $message ) ) {
				$this->noor_starter_templates->append_to_frontend_error_messages( $message );
			}
			if ( apply_filters( 'noor_starter_templates_save_log_files', false ) ) {
				// Add message to log file.
				$log_added = Helpers::append_to_file(
					__( 'New AJAX call!', 'noor-starter-templates' ) . PHP_EOL . $message,
					$this->noor_starter_templates->get_log_file_path(),
					''
				);
			}
	...
	public function add_term_tracking( $term_id, $data ) {
		update_term_meta( $term_id, '_noor_starter_templates_imported_term', true );
	}
	/**
	 * Run elementor Import.
	 *
	...
	public function add_post_tracking( $post_id, $data, $meta, $comments, $terms ) {
		update_post_meta( $post_id, '_noor_starter_templates_imported_post', true );
	}
	...
	private function set_current_importer_data() {
		$data = array_merge( $this->noor_starter_templates->get_current_importer_data(), $this->get_importer_data() );
