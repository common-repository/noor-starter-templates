		...
		if ( is_wp_error( $results ) ) {
			...
			if ( apply_filters( 'noor_starter_templates_save_log_files', false ) ) {
				...
				Helpers::append_to_file(
					$error_message,
					$log_file_path,
					esc_html__( 'Importing customizer settings', 'noor-starter-templates' )
				);
			}
		} else {
			if ( apply_filters( 'noor_starter_templates_save_log_files', false ) ) {
				...
				$log_added = Helpers::append_to_file(
					esc_html__( 'Customizer settings import finished!', 'noor-starter-templates' ),
					$log_file_path,
					esc_html__( 'Importing customizer settings', 'noor-starter-templates' )
				);
			}
	...
	public static function import_customizer_options( $import_file_path ) {
		...
		if ( ! file_exists( $import_file_path ) ) {
			return new \WP_Error(
				...
				sprintf(
					esc_html__( 'Error: The customizer import file is missing! File path: %s', 'noor-starter-templates' ),
					$import_file_path
				)
			);
		}
		...
		if ( 'array' != gettype( $data ) && ( ! isset( $data['template'] ) || ! isset( $data['mods'] ) ) ) {
			return new \WP_Error(
				...
				esc_html__( 'Error: The customizer import file is not in a correct format. Please make sure to use the correct customizer import file.', 'noor-starter-templates' )
			);
		}
		if ( $data['template'] !== $template ) {
			return new \WP_Error(
				'customizer_import_wrong_theme',
				esc_html__( 'Error: The customizer import file is not suitable for current theme. You can only import customizer settings for the same theme or a child theme.', 'noor-starter-templates' )
			);
		}

		// Import images.
		if ( apply_filters( 'noor-starter-templates/customizer_import_images', true ) ) {
		...
		if ( isset( $data['options'] ) ) {
			...
			if ( ! class_exists( 'Noor_Starter_Templates\CustomizerOption' ) ) {
				require_once NOOR_STARTER_TEMPLATES_PATH . 'inc/class-customizer-option.php';
			}
			foreach ( $data['options'] as $option_key => $option_value ) {
				if ( 'dima_global_palette' === $option_key ) {
					$palette = json_decode( $option_value, 'true' );
					$info    = update_option( 'dima_global_palette', json_encode( $palette ) );
					if ( ! $info ) {
						$info = add_option( 'dima_global_palette', json_encode( $palette ) );
					}
				} else {
					if ( str_contains( $option_key, \Dima\dima()->get_option_name() ) ) {
						$key = Helpers::get_string_between( $option_key, '[', ']' );
						\Dima\dima()->update_option( $key, $option_value );
					} else {
		...
		$use_wp_customize_save_hooks = apply_filters( 'noor-starter-templates/enable_wp_customize_save_hooks', false );
		...
		do_action( 'noor-starter-templates/after_customizer_import', $data );
		return true;
	}
	...
	public static function import_customizer_options_only_color( $import_file_path ) {
		...
		if ( isset( $data['options'] ) ) {
			foreach ( $data['options'] as $option_key => $option_value ) {
				if ( 'dima_global_palette' === $option_key ) {
					$palette = json_decode( $option_value, 'true' );
					update_option( 'dima_global_palette', json_encode( $palette ) );
				}
			}
		}
	}
