<?php
/**
 * Static functions used in the Noor Starter Templates plugin.
 *
...
class Helpers {
	...
	public static function download_import_file( $import_file_info, $page_id ) {
		...
		$content_filename = apply_filters( 'dima-starter-templates/downloaded_content_file_prefix', 'demo-content-import-file_' ) . self::$demo_import_start_time . apply_filters( 'dima-starter-templates/downloaded_content_file_suffix_and_file_extension', '.xml' );
		...
		if ( ! empty( $import_file_info['theme_options'] ) ) {
			...
			$customizer_filename = apply_filters( 'dima-starter-templates/downloaded_customizer_file_prefix', 'demo-customizer-import-file_' ) . self::$demo_import_start_time . apply_filters( 'dima-starter-templates/downloaded_customizer_file_suffix_and_file_extension', '.dat' );
	...
	public static function download_import_files( $import_file_info ) {
		...
		if ( ! empty( $import_file_info['widget_data'] ) ) {
			...
			$widget_filename = apply_filters( 'dima-starter-templates/downloaded_widgets_file_prefix', 'demo-widgets-import-file_' ) . self::$demo_import_start_time . apply_filters( 'dima-starter-templates/downloaded_widgets_file_suffix_and_file_extension', '.json' );
		...
		} elseif ( ! empty( $import_file_info['local_widget_data'] ) ) {
		...
		// Slider Revolution.
		if ( ! empty( $import_file_info['sliders_data'] ) ) {
			// Set the filename string for revslider import file.
			$widget_filename = apply_filters( 'dima-starter-templates/downloaded_revslider_file_prefix', 'demo-revslider-import-file_' ) . self::$demo_import_start_time . apply_filters( 'dima-starter-templates/downloaded_revslider_file_suffix_and_file_extension', '.zip' );

			// Download the widgets import file.
			$downloaded_files['revslider'] = $downloader->download_file( $import_file_info['sliders_data'], $widget_filename );

			// Return from this function if there was an error.
			if ( is_wp_error( $downloaded_files['revslider'] ) ) {
				return $downloaded_files['revslider'];
			}
		} elseif ( ! empty( $import_file_info['local_revslider_data'] ) ) {
			if ( file_exists( $import_file_info['local_revslider_data'] ) ) {
				$downloaded_files['revslider'] = $import_file_info['local_revslider_data'];
			}
		}
		...
		if ( ! empty( $import_file_info['theme_options'] ) ) {
			...
			// Download the customizer import file.
			$downloaded_files['customizer'] = $downloader->download_file( $import_file_info['theme_options'], $customizer_filename );
			// \error_log( \print_r( $downloaded_files['customizer'], true ) );
			// Return from this function if there was an error.
			if ( is_wp_error( $downloaded_files['customizer'] ) ) {
				return $downloaded_files['customizer'];
			}
		} elseif ( ! empty( $import_file_info['local_theme_options'] ) ) {
		...
		if ( ! empty( $import_file_info['form_data'] ) ) {
			...
			$form_filename = apply_filters( 'dima-starter-templates/downloaded_forms_file_prefix', 'demo-forms-import-file_' ) . self::$demo_import_start_time . apply_filters( 'dima-starter-templates/downloaded_form_file_suffix_and_file_extension', '.json' );
		...
		} elseif ( ! empty( $import_file_info['local_form_data'] ) ) {
		...
		if ( ! empty( $import_file_info['give_donation_data'] ) ) {
			// Set the filename string for form import file.
			$give_filename = apply_filters( 'dima-starter-templates/downloaded_forms_file_prefix', 'demo-give-donations-import-file_' ) . self::$demo_import_start_time . apply_filters( 'dima-starter-templates/downloaded_give_donations_file_suffix_and_file_extension', '.json' );
		...
		if ( ! empty( $import_file_info['give_form_data'] ) ) {
			// Set the filename string for form import file.
			$give_form_filename = apply_filters( 'dima-starter-templates/downloaded_forms_file_prefix', 'demo-give-forms-import-file_' ) . self::$demo_import_start_time . apply_filters( 'dima-starter-templates/downloaded_give_form_file_suffix_and_file_extension', '.json' );
	...
	public static function write_to_file( $content, $file_path ) {
		...
		if ( ! $wp_filesystem->put_contents( $file_path, $content ) ) {
			return new \WP_Error(
				...
				sprintf(
					__( 'An error occurred while writing file to your server! Tried to write a file to: %1$s%2$s.', 'noor-starter-templates' ),
					'<br>',
					$file_path
				)
			);
	...
	public static function data_from_file( $file_path ) {
		...
		if ( ! $data ) {
			return new \WP_Error(
				...
				sprintf(
					__( 'An error occurred while reading a file from your server! Tried reading file from path: %1$s%2$s.', 'noor-starter-templates' ),
					'<br>',
					$file_path
				)
			);
	...
	private static function check_wp_filesystem_credentials() {
		...
		if ( ! ( 'direct' === get_filesystem_method() ) ) {
			return new \WP_Error(
				...
				sprintf(
					__( 'This WordPress page does not have %1$sdirect%2$s write file access. This plugin needs it in order to save the demo import xml file to the upload directory of your site. You can change this setting with these instructions: %3$s.', 'noor-starter-templates' ),
					'<strong>',
					'</strong>',
		...
		$plugin_page_setup = apply_filters(
			'dima-starter-templates/plugin_page_setup',
			array(
				'parent_slug' => 'themes.php',
				'page_title'  => esc_html__( 'One Click Demo Import', 'noor-starter-templates' ),
				'menu_title'  => esc_html__( 'Import Demo Data', 'noor-starter-templates' ),
				'capability'  => 'import',
		...
		if ( false === ( $creds = request_filesystem_credentials( $demo_import_page_url, '', false, false, null ) ) ) {
			return new \WP_error(
				...
				__( 'An error occurred while retrieving reading/writing permissions to your server (could not retrieve WP filesystem credentials)!', 'noor-starter-templates' )
			);
		}
		...
		if ( ! WP_Filesystem( $creds ) ) {
			return new \WP_Error(
				...
				__( 'Your WordPress login credentials don\'t allow to use WP_Filesystem!', 'noor-starter-templates' )
			);
		}
	...
	public static function get_log_path() {
		...
		$upload_path = apply_filters( 'dima-starter-templates/upload_file_path', trailingslashit( $upload_dir['path'] ) );

		$log_path = $upload_path . apply_filters( 'dima-starter-templates/log_file_prefix', 'log_file_' ) . self::$demo_import_start_time . apply_filters( 'dima-starter-templates/log_file_suffix_and_file_extension', '.log' );
	...
	public static function register_file_as_media_attachment( $log_path ) {
		...
		$filetype  = wp_check_filetype( basename( $log_path ), apply_filters( 'dima-starter-templates/file_mimes', $log_mimes ) );

		// Prepare an array of post data for the attachment.
		$attachment = array(
			'guid'           => self::get_log_url( $log_path ),
			'post_mime_type' => $filetype['type'],
			'post_title'     => apply_filters( 'dima-starter-templates/attachment_prefix', esc_html__( 'One Click Demo Import - ', 'noor-starter-templates' ) ) . preg_replace( '/\.[^.]+$/', '', basename( $log_path ) ),
			'post_content'   => '',
			'post_status'    => 'inherit',
		);
	...
	public static function get_log_url( $log_path ) {
		$upload_dir = wp_upload_dir();
		$upload_url = apply_filters( 'dima-starter-templates/upload_file_url', trailingslashit( $upload_dir['url'] ) );
	...
	public static function verify_ajax_call() {
		check_ajax_referer( 'noor-ajax-verification', 'security' );

		// Check if user has the WP capability to import data.
		if ( ! current_user_can( 'import' ) ) {
			wp_die(
				sprintf(
					__( '%1$sYour user role isn\'t high enough. You don\'t have permission to import demo data.%2$s', 'noor-starter-templates' ),
	...
	public static function process_uploaded_files( $uploaded_files, $log_file_path ) {
		...
		$file_not_provided_error = array(
			'error' => esc_html__( 'No file provided.', 'noor-starter-templates' ),
		);
		...
		if ( $content_file_info && ! isset( $content_file_info['error'] ) ) {
			...
			if ( apply_filters( 'noor_starter_templates_save_log_files', false ) ) {
				...
				$log_added = self::append_to_file(
					sprintf(
						__( 'Content file was not uploaded. Error: %s', 'noor-starter-templates' ),
						$content_file_info['error']
					),
					$log_file_path,
					esc_html__( 'Upload files', 'noor-starter-templates' )
				);
			}
		}
		...
		if ( $widget_file_info && ! isset( $widget_file_info['error'] ) ) {
			...
			if ( apply_filters( 'noor_starter_templates_save_log_files', false ) ) {
				...
				$log_added = self::append_to_file(
					sprintf(
						__( 'Widget file was not uploaded. Error: %s', 'noor-starter-templates' ),
		...
		if ( $customizer_file_info && ! isset( $customizer_file_info['error'] ) ) {
			...
			if ( apply_filters( 'noor_starter_templates_save_log_files', false ) ) {
				...
				$log_added = self::append_to_file(
					sprintf(
						__( 'Customizer file was not uploaded. Error: %s', 'noor-starter-templates' ),
		...
		if ( $redux_file_info && ! isset( $redux_file_info['error'] ) ) {
			if ( isset( $_POST['redux_option_name'] ) && empty( $_POST['redux_option_name'] ) ) {
				...
				self::log_error_and_send_ajax_response(
					esc_html__( 'Missing Redux option name! Please also enter the Redux option name!', 'noor-starter-templates' ),
					$log_file_path,
					esc_html__( 'Upload files', 'noor-starter-templates' )
				);
			}
			...
			if ( apply_filters( 'noor_starter_templates_save_log_files', false ) ) {
				...
				$log_added = self::append_to_file(
					sprintf(
						__( 'Redux file was not uploaded. Error: %s', 'noor-starter-templates' ),
		...
		if ( apply_filters( 'noor_starter_templates_save_log_files', false ) ) {
			...
			$log_added = self::append_to_file(
				__( 'The import files were successfully uploaded!', 'noor-starter-templates' ) . self::import_file_info( $selected_import_files ),
				$log_file_path,
				esc_html__( 'Upload files', 'noor-starter-templates' )
			);
		}
	...
	public static function import_file_info( $selected_import_files ) {
		...
		return PHP_EOL .
		sprintf(
			__( 'Initial max execution time = %s', 'noor-starter-templates' ),
			ini_get( 'max_execution_time' )
		) . PHP_EOL .
		sprintf(
			__( 'Files info:%1$sSite URL = %2$s%1$sData file = %3$s%1$sWidget file = %4$s%1$sCustomizer file = %5$s%1$sForm File:%1$s%6$s', 'noor-starter-templates' ),
			PHP_EOL,
			get_site_url(),
			empty( $selected_import_files['content'] ) ? esc_html__( 'not defined!', 'noor-starter-templates' ) : $selected_import_files['content'],
			empty( $selected_import_files['widgets'] ) ? esc_html__( 'not defined!', 'noor-starter-templates' ) : $selected_import_files['widgets'],
			empty( $selected_import_files['customizer'] ) ? esc_html__( 'not defined!', 'noor-starter-templates' ) : $selected_import_files['customizer'],
			empty( $selected_import_files['forms'] ) ? esc_html__( 'not defined!', 'noor-starter-templates' ) : $selected_import_files['forms']
		);
	}
	...
	public static function set_demo_import_start_time() {
		self::$demo_import_start_time = date( apply_filters( 'dima-starter-templates/date_format_for_file_names', 'Y-m-d__H-i-s' ) );
	}
	...
	/**
	 * Set the Noor transient with the current importer data.
	 *
	 * @param array $data Data to be saved to the transient.
	 */
	public static function set_import_data_transient( $data ) {
		set_transient( 'noor_importer_data', $data, 0.1 * HOUR_IN_SECONDS );
	}
}
